    ...
    @IBAction func addEvent(sender: UIButton) {
        ...
        if (EKEventStore.authorizationStatusForEntityType(.Event) != EKAuthorizationStatus.Authorized) {
            eventStore.requestAccessToEntityType(.Event, completion: {
                granted, error in
                self.createEvent(eventStore, title: "DggibuJ's Test Event", startDate: startDate, endDate: endDate)
            })
        } else {
            createEvent(eventStore, title: "DhbjhhjhjkhjJ's Test Event", startDate: startDate, endDate: endDate)
        }
    }
